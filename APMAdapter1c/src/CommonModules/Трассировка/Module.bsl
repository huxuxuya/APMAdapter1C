...
Процедура ДобавитьОтрезок(Событие, НазваниеСобытия) Экспорт
	...
	ДанныеОтрезка.Вставить("duration", 100.1);
...
Процедура ЗакрытьПоследнийОтрезок(Событие) Экспорт 
	...
	ПоследнийОтрезок = Событие.spans[КоличествоСобытий-1];

	Если ПоследнийОтрезок.start <> 0 И ПоследнийОтрезок.duration <> 100.1 Тогда
		// Отрезок уже закрыт
		Возврат;
	КонецЕсли;
	
	ПоследнийОтрезок.start = ПоследнийОтрезок.timestamp - Событие.transaction.timestamp; 
	
	ПоследнийОтрезок.duration = (ТекущаяУниверсальнаяДатаВМикросекундахОтЮниксЭпохи() - ПоследнийОтрезок.timestamp) / 1000;; 
	
КонецПроцедуры

Процедура ЗакрытьОтрезок(Событие, Индекс) Экспорт
	
	Отрезок = Событие.spans[Индекс];
	
	Отрезок.duration = (ТекущаяУниверсальнаяДатаВМикросекундахОтЮниксЭпохи() - Отрезок.timestamp) / 1000; 
	
	Отрезок.start = ТекущаяУниверсальнаяДатаВМикросекундахОтЮниксЭпохи() - (Отрезок.duration*1000); 
...
Функция СериализоватьСобытие(Знач Событие) Экспорт
